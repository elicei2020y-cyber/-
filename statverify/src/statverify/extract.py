...
# Утверждение обрывается там, где начинается СЛЕДУЮЩИЙ t(...) — тот же
# принцип, что уже несёт _GAP внутри TIGHT (не пересекать вложенный
# t-тест ради p). Используется в _claim_from_loose, чтобы решить границу
# утверждения ДО поиска кандидата p, а не по его следу.
_NEXT_T = re.compile(r"\bt\s*\(")
...
def _claim_from_loose(source: str, m: re.Match) -> Claim:
    ...
    Граница утверждения (span) решена НЕЗАВИСИМО от того, что внутри
    найдётся: конец окна поиска — это либо CANDIDATE_WINDOW символов,
    либо начало следующего t(...), какое из двух раньше, и решается
    ДО поиска p. Раньше span_end брался как max() позиций НАЙДЕННЫХ
    кандидатов — тогда C0 в guard.py ('кандидат внутри границ
    утверждения') была тождественно истинной по построению: граница
    подстраивалась под то, что в неё же и проверялось. Из-за этого
    p из вклинившейся соседней колонки (у которой раньше своего p
    встречается собственный t(...)) свободно проходил внутрь.
    Теперь такой сплайс обрубает окно поиска на границе чужого
    t(...), и до чужого p извлекатель просто не доходит — LOOSE найдёт
    0 кандидатов для исходного утверждения, что честно даёт отказ,
    а не тихо неверное значение.

    ОСТАТОЧНЫЙ РИСК, названный прямо, а не спрятанный: граница не
    ловит вклинившийся фрагмент БЕЗ собственного t(...) внутри —
    например, чистую прозу со случайным 'p = .05'. Признака конца
    предложения, независимого от вёрстки, в этом извлекателе нет.
    """
    end = m.end()
    next_t = _NEXT_T.search(source, end, end + CANDIDATE_WINDOW)
    limit = (next_t.start() - end) if next_t else CANDIDATE_WINDOW
    window = source[end:end + limit]
    cands: List[Slot] = []
