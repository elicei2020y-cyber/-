...
# SPSS и часть журналов печатают 'p = .001' для всего, что меньше порога,
# — не ошибка авторов, а способ печати. Найдено на реальном корпусе
# (docs/CORPUS_REAL_NOTES.md): 5 из 8 расхождений на 49 утверждениях из
# настоящих статей PLOS ONE именно такие.
#
# Порог не перечислен списком (.05/.01/.001/.0001) — список содержал .05,
# и это была настоящая ошибка: .05 не порог печати, это уровень значимости,
# а запись 'p = .05' при вычисленном .02 — либо опечатка, либо подгонка
# результата под порог значимости, то есть самый интересный класс находок,
# который список тихо гасил бы под видом технической печати.
#
# Порог выводится, а не перечисляется: печать нижнего порога — ровно
# наименьшее ПРЕДСТАВИМОЕ ненулевое значение при точности записи p,
# то есть 10^-p_decimals. Это воспроизводит .001 (3 знака) и .0001
# (4 знака) без перечисления, само исключает .05 и .002 (они не являются
# наименьшим представимым числом при своей точности — 'p = .05' с двумя
# знаками означало бы порог .01, не .05), и не потребует правки для
# журнала, печатающего 'p < .00001'.
...
def _floor_convention(claim: Claim) -> Optional[float]:
    ...
    отношение записано как '=', сообщённый p равен наименьшему
    представимому ненулевому числу при записанной точности (10^-p_decimals
    — печать порога, а не перечисление конкретных значений, см. комментарий
    выше), и точно вычисленный двусторонний p по заявленным (точечным, не
    интервальным) t и df строго меньше сообщённого. Сравнение здесь
    намеренно точечное, не интервальное: вопрос не «пересекаются ли
    ...
    if p_slot.relation != "=":
        return None
    threshold = 10.0 ** -p_slot.decimals
    if abs(p_slot.value - threshold) > 1e-12:
        return None
