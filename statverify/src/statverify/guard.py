...
#
# ОСТАТОЧНЫЙ РИСК, принятый сознательно: 'p = .016\n3 participants' даст
# ложный отказ — настоящее число '.016' завершено, но следом идёт
# перенос строки и цифра ('3'), которая на самом деле начинает новое
# предложение, а не продолжает число. Отличить от настоящего обрыва
# без просмотра дальше первой цифры нельзя. Выбор — в пользу отказа:
# он восстановим повторным извлечением, а принятое по ошибке
# усечённое число — нет.
_LINE_ARTIFACT = re.compile(r"^\.?(?:\n|­|-\n)+\d")
