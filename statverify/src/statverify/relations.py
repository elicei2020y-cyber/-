...
def _df_from_ciw(k):
    ...
    # инвертирует функцию с горизонтальной асимптотой (t_crit(df,alpha) ->
    # const при df -> inf, никогда её не достигая), и Relation.solve
    # раньше молча отбрасывал углы бокса, где решатель возвращал None,
    # беря min/max только по успешным углам. Для df, близких к асимптотике
    # (df ≳ 25 при alpha=.05), это давало заведомо узкий интервал, не
    # содержащий истинный df (проверено: при df=99, est=0.2, se=0.1
    # решение давало [14.2, 32.4]). Из-за этого направление было убрано.
    #
    # Проблема была не в самой формуле (она монотонна по каждому
    # аргументу и всюду корректна), а в Relation.solve — тот факт, что
    # решатель не определён на всём боксе, solve теперь трактует как
    # «нет ограничения» (расширяет до области определения), а не молча
