...
CORPUS: List[Dict[str, Any]] = [
    ...
    dict(id="sentence_break", text="The main effect held, t(99) = 2.45. This was reliable, p = .016.",
         claims=[dict(t=2.45, df=99.0, p=0.016, prel="=")]),
    # ИСТОРИЯ ЭТОГО СЛУЧАЯ, ЧЕСТНО. Между исходным и текущим ожиданием
    # был промежуточный раунд, где это ожидание ставилось в p=None:
    # тогдашняя починка склейки колонок (extract._claim_from_loose)
    # запрещала продвижение единственного кандидата при ЛЮБОМ переносе
    # строки между t-образцом и им — не различая обычный перенос внутри
    # предложения (в PDF происходит постоянно) и вытекание чужой
    # колонки. Ревью верно указало: этот признак ложно топит и
    # легитимные случаи вроде этого, а corpus_real.py (получен из HTML)
    # физически не может об этом свидетельствовать — там переносов
    # строк нет вовсе, поэтому "потерь на корпусе нет" не проверяло то,
    # что нужно. Действующая починка различает случаи ЧИСЛОМ кандидатов,
    # а не наличием переноса: при вытекании колонки настоящий p остаётся
    # в тексте ДАЛЬШЕ вклинившегося фрагмента, и расширенный поиск
    # (WIDE_CANDIDATE_WINDOW, extract.py) находит его вторым кандидатом
    # — то есть настоящую склейку ловит AMBIGUOUS_PARSE от двух
    # найденных значений, а не запрет по переносу. Здесь второго p нет
    # ни в обычном, ни в расширенном окне, поэтому ожидание вернулось
    # к исходному — успешное извлечение.
